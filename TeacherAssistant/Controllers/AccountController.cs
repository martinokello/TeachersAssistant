...
namespace  TeacherAssistant.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(string returnUrl)
        {
            ...
            if(string.IsNullOrEmpty(Request.QueryString["Role"]) && Session["Role"] == null)
            {
                return View();
            }
            ...
            var model = new LoginViewModel { Role = Session["Role"] as string };

            if (!string.IsNullOrEmpty(model.Role) && User.Identity.IsAuthenticated && User.IsInRole(model.Role) && !model.Role.ToLower().Equals("administrator"))
            {
                return RedirectToLocal("~/" + model.Role + "/Home/Index");
            }
        ...
        private string DoSwitch(string role)
        {
            role = role ?? "";
            ...
            {
                case "grammar11plus":
                    returnUrl = "~/Grammar11Plus/Home/Index";
                    break;
                case "statejunior":
                    returnUrl = "~/StateJunior/Home/Index";
                    break;
                case "stateprimary":
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            if (User.Identity.IsAuthenticated && Roles.IsUserInRole(model.Role))
            {
            ...
            switch (result)
            {
                ...
                case SignInStatus.Failure:
                    ...
                    return View(model);
                default: return RedirectToLocal("~/Home/Index");
            }
        }
        ...
        private void InitializeDefaultAdminUser()
        {
            ...
            if (aspUser == null)
            {
                var user = new ApplicationUser { UserName = defaultAdmin, Email = defaultAdmin };
                var result = UserManager.Create(user, defaultAdminPassword);
                Roles.AddUserToRole(defaultAdmin, "Administrator");
            }
        }
        //
